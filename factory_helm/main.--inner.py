...
class helmClass(TerraformStack):
    ...
    def _setKubeconfig(self,path):
        self.config_data["kubernetes"]["config_path"] = path           
        
    def deploy_value(self, helm_name, helm_values_data ):        
        print("helm value:", helm_values_data)
        self.resource_factory.create_resource(Release, helm_name, **helm_values_data)
    
    def deploy(self, depoy_auto_applove = False ):                
    
        name = self.getHelmInfo()["name"]
        config_info = self.getHelmInfo()                
        self.resource_factory.create_resource(Release,  name,**(config_info ) )
        #self.app.synth()
        ...
        if depoy_auto_applove == CDK_DEPLOY :
            #print( "DEFLOY")
            shell = ShellCommandDB( 100,1000, 'cdktf diff') 
            sCapture = shell.Excute_Capture_All_Value()
            
            
        elif  depoy_auto_applove == CDK_DEPLOY_AUTOAPPROVE :
            shell = ShellCommandDB( 100,1000, 'cdktf deploy --auto-apporve') 
            sCapture = shell.Excute_Capture_All_Value()
    ...
    def setHelmInfo( self,dataMap):
        self.release_config_data = dataMap
        
    def getHelmInfo( self ):
        return self.release_config_data
        
    def setHelmInfoValue( self, lst_value):
        self.release_config_data["values"] = [lst_value]
