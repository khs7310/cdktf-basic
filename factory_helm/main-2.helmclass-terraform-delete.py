...
class ResourceFactory(TerraformStack):
    helmProvider = None
    providerId = "helmProvider"
    app = None
    
    def __init__(self, stack_app, provider_config):            
        
        super().__init__(scope = stack_app, id=self.providerId)
        self.app = stack_app
        self.provider_config = provider_config
        self.helmProvider = HelmProvider( scope = stack_app ,id = "helm" ,  **self.provider_config ) 
        
        
        

    # def create_resource(self,   **resource_data):
    #     resource_helm = Release( provider=self.helmProvider, **resource_data)
...
class ResourceFactory_T():
    ...
    def __init__(self,  provider_config ):            
        ...
        #super().__init__(scope = self.app, id=self.providerId)
        
        self.provider_config = provider_config
        #self.helmProvider = HelmProvider( scope = self.app ,id = "helm" ,  **self.provider_config ) 
...
class helmClass():
    ...
    def helm_deploy(self, resource_helm_name:str, ** resource_data):
        
        #self.resource_factory = ResourceFactory( self.stack_app,self.kube_config_data)
       
            
        self.resource_factory.create_resource(resource_helm_name, **resource_data)
        #provider = HelmProvider( self, self.id , **self.config_data )
        
        # sleep(1)
        # self.resource_factory.create_resource(self, **self.release_config_data)
        # #helm_release = Release(provider, self.id,  **self.release_config_data)
        # self.app.synth()
...
release_config_data = {
    "chart": "nginx",
    "name": "cdktf-nginx9",   #<<== resource.instances.attribute.id | attrribute.name || resource.instance.name
...
release_apache_config_data = {
    "chart": "apache",
    "name": "cdktf-apache",   #<<== resource.instances.attribute.id | attrribute.name || resource.instance.name
    "create_namespace" : True,
    "namespace" : "cdktf-nginx",
    "repository" : "https://charts.bitnami.com/bitnami",
    "version": "9.6.3",
...
helm = helmClass( )
helm.helm_deploy( "helm_name_thing", **release_config_data  )
helm.helm_deploy( "helm_name_tomcat", **release_tomcat_config_data  )
helm.helm_deploy( "helm_name_apache", **release_apache_config_data  )
